...
不单开一个「会话面板」—— 会话本来就属于项目，单开一个窗口等于让人先想
「我要看会话」再想「是哪个项目的」。

「新对话」不会把旧对话弄丢：pi 把每个会话存成独立的 `.jsonl` 文件，
`new_session` 只换了 pi 内存里的当前会话，磁盘上一个字节都没动。
只是 pi 的 RPC 里**没有「列出会话」的命令**（它 TUI 的 picker 不对外），
所以列表是 Pi GUI 自己扫 `<agentDir>/sessions/` 得到的。

**归属只认会话文件第一行的 `cwd`**，不认目录名 —— 目录名是 pi 的实现细节，
它变了也不该让两个项目的对话串在一起。

### 新会话为什么显示成斜体

pi 在会话**还没有任何内容时不落盘**：点了「新对话」之后 pi 已经切到新会话，
但那个文件要等你发出第一条消息才写。所以列表里会出现一条斜体的
「新会话（还没有消息）」——那是 Pi GUI 按 pi 报的当前会话补上去的占位，
它没有对应的磁盘文件，因此**不能切换、不能归档、不能删除**。
发出第一条消息后它就会变成正常的一条。

### 归档

归档是**可逆的组织动作**：会话文件原地不动，pi 那边完全不知道，
只影响 Pi GUI 这个列表。所以归档不会改变 `pi --continue` 会接着哪个会话。

被归档的收进「已归档 N 条」折叠组，点开才看得到。**正在进行的会话不能归档**
—— 先切到别的会话再归档。
...
一条会话被删掉之后就不在列表里了，但磁盘上的那份还在回收站里。

### 边界

- 列表响应里**不含任何绝对路径**：前端只拿得到稳定 ID（路径的 sha1 前 16 位），
  切换 / 归档 / 删除都只传 ID，由后端在自己的索引里解析真实路径，
  并核对那条会话的 `cwd` 确实属于当前项目 ⇒ 不可能操作到别的项目的会话上。
- 归档与回收站是 Pi GUI 自己的状态，**不往 pi 的目录里塞任何东西**。
- 会话文件读不出来（坏 JSON、被别的程序占着）只影响它自己，在列表里跳过并给一条诊断。
...
npm run test:sessions   # 会话：目录名编码、单个会话文件的解析（含两种消息体形状）、
                        #   列表与归属判定（只认 header.cwd）、安全边界（未知 id /
                        #   路径穿越 / 跨项目 id 一律拒绝）、切换与改名的降级、
                        #   归档与删除（当前会话拒绝、软删除进回收站、flags 原子写、
                        #   坏 flags 不拖垮列表）、「当前会话还没落盘时补位」、HTTP 路由。
                        #   全程 os.tmpdir()，**绝不碰真实的 ~/.pi/agent/sessions**
...
- `server/` — 后端各职责模块。全部由 `server.js` 装配，模块之间不互相 import
  ...
  - `sessions.js` — 会话列表 / 切换 / 改名 / 归档 / 删除。pi 的 RPC 有
    `switch_session` 但**没有「列出会话」**，所以列表由这里扫 `<agentDir>/sessions/`，
    归属判定**只认会话文件 header 里的 cwd**。前端只拿得到稳定 ID（路径 sha1 前 16 位），
    拿不到也传不了绝对路径。归档与回收站是 Pi GUI 自己的状态，落在 `<PI_GUI_DATA>/`，
    **不往 pi 的目录里塞东西**；删除是软删除（文件移进 `trash-sessions/`，不是 unlink）
...
- `public/` — 前端。原生 ES Module，`app.js` 只做装配，其余按职责分模块
  ...
  - `sessions.js` — 侧栏的会话列表。会话挂在**当前项目那一行下面**（参考 Codex，
    不单开窗口），支持切换 / 行内改名 / 归档 / 删除。行内动作默认透明、悬停才显形。
    ⚠️ 会话一变（新开 / 分叉 / 切换）必须重画这块列表 —— 否则它会停在旧状态，
    旧会话仍然带着「当前」标记，而当前项是不给点的，用户就再也回不到那条对话
  - `conversation-nav.js` — 会话内提问导航（聊天区左边一列短线）。**只画用户消息**；
    DOM 是唯一权威，历史重建 / 分叉 / 重试 / 切项目后一律重新扫 DOM，不存消息副本
  - `tree.js` — 分支树。pi 的会话是一棵 append-only 的条目树，每个节点都能「从此处
    分叉」。只列**参与对话的条目**（`message` / `custom_message`）—— 切换模型、
    调整思考档位这类操作不列；隐藏节点的子节点会上提到最近的可见祖先。
    全部左对齐，只在分叉点标一下支线数
